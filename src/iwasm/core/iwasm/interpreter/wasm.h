...
typedef struct ClusterMapping {
    uint32_t num_expressions;  // num of expr
    uint32_t *expression_indices;  //  expr index vector
} ClusterMapping;

typedef struct MappingData {
    uint32_t num_clusters;  // cluster vector
    ClusterMapping *clusters;  // cluster mapping vector
} MappingData;

typedef struct Expression {
    uint32_t size;       // Size 
    uint8_t *encoding;   // Pointer to the expression encoding data
} Expression;

typedef struct ExprData {
    uint32_t num_expressions;  // Number of expressions
...
struct WASMModule {
    ...
    /* self-defined wasm encoding section aka custom section*/
    uint8 *encoding_section;
    uint32 encoding_section_size;

    uint8 *cluster_section;
    uint32 cluster_section_size;
    ClusterData *cluster_data;

    uint8 *mapping_section;
    uint32 mapping_section_size;
    MappingData *mapping_data;

    uint8 *expr_section;
    uint32 expr_section_size;
    ExprData *expr_data;
