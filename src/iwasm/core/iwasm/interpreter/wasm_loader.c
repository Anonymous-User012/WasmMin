                  ...
                  uint32 error_buf_size)
{
    ...
    if (table_index != 0) {
        set_error_buf(error_buf, error_buf_size, "zero byte expected");
        set_error_buf(error_buf, error_buf_size, "zero byte expecte1");
        return false;
    }
#endif
...
static bool parseExprSection(ExprData *expr_data, uint8 *expr_section, uint32 content_size, char *error_buf, uint32 error_buf_size) {
    if (content_size < 4) {
        set_error_buf(error_buf, error_buf_size, "Invalid expr section: too small");
        return false;
    }

    uint8 *p = expr_section;
    uint8 *p_end = expr_section + content_size;

    // get the number of expressions
    uint32_t num_expressions = *((uint32_t *)p);
    p += sizeof(uint32_t);

#ifdef DEBUG
    // printf("Parsing expr section: %u expressions\n", num_expressions);
#endif

    // ExprData
    expr_data->num_expressions = num_expressions;
    expr_data->expressions = (Expression *)loader_malloc(num_expressions * sizeof(Expression), error_buf, error_buf_size);
    if (!expr_data->expressions) {
        set_error_buf(error_buf, error_buf_size, "Failed to allocate memory for expressions");
        return false;
    }
    ...
    for (uint32_t i = 0; i < num_expressions; i++) {
        if (p + 4 > p_end) {
            set_error_buf(error_buf, error_buf_size, "Invalid expr section: insufficient data for expression size");
            return false;
        }

        // read the size of the expression
        uint32_t expr_size = *((uint32_t *)p);
        p += sizeof(uint32_t);

        if (p + expr_size > p_end) {
            set_error_buf(error_buf, error_buf_size, "Invalid expr section: expression size exceeds section bounds");
            return false;
        }

        // allocate memory for expression encoding
        expr_data->expressions[i].size = expr_size;
        expr_data->expressions[i].encoding = (uint8_t *)loader_malloc(expr_size, error_buf, error_buf_size);
        if (!expr_data->expressions[i].encoding) {
            set_error_buf(error_buf, error_buf_size, "Failed to allocate memory for expression encoding");
            return false;
        }

        // read the expression encoding
        bh_memcpy_s(expr_data->expressions[i].encoding, expr_size, p, expr_size);
        p += expr_size;

#ifdef DEBUG
        printf("OP idx : %u -> size %u, encoding: ", i, expr_size);
        for (uint32_t j = 0; j < expr_size; j++) {
            printf("%02X ", expr_data->expressions[i].encoding[j]);
        }
        printf("\n");
#endif        
    }

    LOG_VERBOSE("Successfully parsed expr section.");
    return true;
}



static bool parseClusterSection(ClusterData *cluster_data, uint8 *cluster_section, uint32 content_size, char *error_buf, uint32 error_buf_size) {
    if (content_size < 4) {
        set_error_buf(error_buf, error_buf_size, "Invalid cluster section: too small");
        return false;
    }

    uint8 *p = cluster_section;
    uint8 *p_end = cluster_section + content_size;

    // cluster 
    uint32_t num_clusters = *((uint32_t *)p);
    p += sizeof(uint32_t);

#ifdef DEBUG
    printf("Parsing cluster section: %u clusters\n", num_clusters);
#endif 

    // ClusterData 
    cluster_data->num_clusters = num_clusters;
    cluster_data->clusters = (Cluster *)loader_malloc(num_clusters * sizeof(Cluster), error_buf, error_buf_size);
    if (!cluster_data->clusters) {
        set_error_buf(error_buf, error_buf_size, "Failed to allocate memory for clusters");
        return false;
    }
    ...
    for (uint32_t i = 0; i < num_clusters; i++) {
        if (p + 4 > p_end) {
            set_error_buf(error_buf, error_buf_size, "Invalid cluster section: insufficient data");
            return false;
        }

        // function 
        uint32_t num_functions = *((uint32_t *)p);
        p += sizeof(uint32_t);

#ifdef DEBUG
        printf("Cluster %u: %u functions\n", i, num_functions);
#endif 

        //  function 
        cluster_data->clusters[i].num_functions = num_functions;
        cluster_data->clusters[i].functions = (uint32_t *)loader_malloc(num_functions * sizeof(uint32_t), error_buf, error_buf_size);
        if (!cluster_data->clusters[i].functions) {
            set_error_buf(error_buf, error_buf_size, "Failed to allocate memory for functions");
            return false;
        }

        // function indices
        for (uint32_t j = 0; j < num_functions; j++) {
            if (p + 4 > p_end) {
                set_error_buf(error_buf, error_buf_size, "Invalid cluster section: insufficient function indices");
                return false;
            }

            cluster_data->clusters[i].functions[j] = *((uint32_t *)p);
            p += sizeof(uint32_t);

#ifdef DEBUG
            printf("  Function Index %u: %u\n", j, cluster_data->clusters[i].functions[j]);
#endif 
        }
    }

    LOG_VERBOSE("Successfully parsed cluster section.");
    return true;
}

static bool parseMappingSection(MappingData *mapping_data, uint8 *mapping_section, uint32 content_size, char *error_buf, uint32 error_buf_size) {
    if (content_size < 4) {
        set_error_buf(error_buf, error_buf_size, "Invalid mapping section: too small");
        return false;
    }

    uint8 *p = mapping_section;
    uint8 *p_end = mapping_section + content_size;
...
#ifdef DEBUG
    printf("Parsing mapping section: %u clusters\n", num_clusters);     
#endif


    //  MappingData 
    mapping_data->num_clusters = num_clusters;
    mapping_data->clusters = (ClusterMapping *)loader_malloc(num_clusters * sizeof(ClusterMapping), error_buf, error_buf_size);
    if (!mapping_data->clusters) {
        set_error_buf(error_buf, error_buf_size, "Failed to allocate memory for clusters");
        return false;
    }
    ...
    for (uint32_t i = 0; i < num_clusters; i++) {
        if (p + 4 > p_end) {
            set_error_buf(error_buf, error_buf_size, "Invalid mapping section: insufficient data for cluster expressions count");
            return false;
        }

        // read the number of exprs within this cluster 
        uint32_t num_expressions = *((uint32_t *)p);
        p += sizeof(uint32_t);


#ifdef DEBUG
        printf("Cluster %u: %u expressions\n", i, num_expressions);
#endif 

        // alloc the number of expression index 
        mapping_data->clusters[i].num_expressions = num_expressions;
        mapping_data->clusters[i].expression_indices = (uint32_t *)loader_malloc(num_expressions * sizeof(uint32_t), error_buf, error_buf_size);
        if (!mapping_data->clusters[i].expression_indices) {
            set_error_buf(error_buf, error_buf_size, "Failed to allocate memory for expression indices");
            return false;
        }

        // read expression indices
        for (uint32_t j = 0; j < num_expressions; j++) {
            if (p + 4 > p_end) {
                set_error_buf(error_buf, error_buf_size, "Invalid mapping section: insufficient expression indices");
                return false;
            }

            mapping_data->clusters[i].expression_indices[j] = *((uint32_t *)p);
            p += sizeof(uint32_t);


#ifdef DEBUG
            printf("  Op: %u -> exprID : %u\n", j, mapping_data->clusters[i].expression_indices[j]);
#endif 
        }
    }

    LOG_VERBOSE("Successfully parsed mapping section.");
    return true;
}
                  ...
                  uint32 error_buf_size)
{
    ...
    // cluster section
    if (name_len == 7 && memcmp(p, "cluster", 7) == 0) {
        uint32 content_size = (uint32)(p_end - p - name_len);
        uint8 *cluster_raw_data = loader_malloc(content_size, error_buf, error_buf_size);
        
        if (!cluster_raw_data) {
            set_error_buf(error_buf, error_buf_size, "allocate memory for cluster section failed");
            return false;
        }

        // 
        bh_memcpy_s(cluster_raw_data, content_size, p + name_len, content_size);
        
        // 
        module->cluster_section = cluster_raw_data;
        module->cluster_section_size = content_size;

        module->cluster_data = wasm_runtime_malloc(sizeof(ClusterData));
        if (!module->cluster_data) {
            set_error_buf(error_buf, error_buf_size, "allocate memory for cluster data failed");
            return false;
        }

        bool success = parseClusterSection(module->cluster_data, cluster_raw_data, content_size, error_buf, error_buf_size);

        if (!success) {
            wasm_runtime_free(module->cluster_data);
            module->cluster_data = NULL;
        }

#ifdef DEBUG
        printf("Found cluster section, size: %u\n", content_size);
        printf("Content: ");
        for (uint32 i = 0; i < content_size; i++) {
            printf("%02x ", cluster_raw_data[i]);
        }
        printf("\n");
#endif
        
        LOG_VERBOSE("Load custom cluster section success.");
    }

    // mapping section
    if (name_len == 7 && memcmp(p, "mapping", 7) == 0) {
        uint32 content_size = (uint32)(p_end - p - name_len);
        uint8 *mapping_raw_data = loader_malloc(content_size, error_buf, error_buf_size);

        if (!mapping_raw_data) {
            set_error_buf(error_buf, error_buf_size, "allocate memory for mapping section failed");
            return false;
        }

        bh_memcpy_s(mapping_raw_data, content_size, p + name_len, content_size);

        module->mapping_section = mapping_raw_data;
        module->mapping_section_size = content_size;

        //  mapping section
        module->mapping_data = wasm_runtime_malloc(sizeof(MappingData));
        if (!module->mapping_data) {
            set_error_buf(error_buf, error_buf_size, "allocate memory for mapping data failed");
            return false;
        }

        bool success = parseMappingSection(module->mapping_data, mapping_raw_data, content_size, error_buf, error_buf_size);

        if (!success) {
            wasm_runtime_free(module->mapping_data);
            module->mapping_data = NULL;
        }

        LOG_VERBOSE("Load custom mapping section success.");
    }



    // expr section
    if (name_len == 4 && memcmp(p, "expr", 4) == 0) {
        uint32 content_size = (uint32)(p_end - p - name_len);
        uint8 *expr_raw_data = loader_malloc(content_size, error_buf, error_buf_size);

        if (!expr_raw_data) {
            set_error_buf(error_buf, error_buf_size, "allocate memory for expr section failed");
            return false;
        }

        bh_memcpy_s(expr_raw_data, content_size, p + name_len, content_size);

        module->expr_section = expr_raw_data;
        module->expr_section_size = content_size;

        // expr section
        module->expr_data = wasm_runtime_malloc(sizeof(ExprData));
        if (!module->expr_data) {
            set_error_buf(error_buf, error_buf_size, "allocate memory for expr data failed");
            return false;
        }

        bool success = parseExprSection(module->expr_data, expr_raw_data, content_size, error_buf, error_buf_size);
        
        if (!success) {
            wasm_runtime_free(module->expr_data);
            module->expr_data = NULL;
        }

        LOG_VERBOSE("Load custom expr section success.");
    }
    ...
    // bool cluster_data_valid = (module && module->cluster_data && 
    //                           module->cluster_data->clusters) ? true : false;
    // bool mapping_data_valid = (module && module->mapping_data && 
    //                           module->mapping_data->clusters) ? true : false;
    // bool expr_data_valid = (module && module->expr_data && 
    //                        module->expr_data->expressions) ? true : false;
    
    // printf("after load custom section: cluster_data_valid: %d, mapping_data_valid: %d, expr_data_valid: %d\n",
    //        cluster_data_valid ? 1 : 0, mapping_data_valid ? 1 : 0, expr_data_valid ? 1 : 0);
                   ...
                   bool no_resolve, char *error_buf, uint32 error_buf_size)
{
    ...
    // bool cluster_data_valid = (module && module->cluster_data) ? true : false;
    // bool mapping_data_valid = (module && module->mapping_data) ? true : false;
    // bool expr_data_valid = (module && module->expr_data) ? true : false;
    
    // printf("before prepare bytecode1: cluster_data_valid: %d, mapping_data_valid: %d, expr_data_valid: %d\n",
    //        cluster_data_valid ? 1 : 0, mapping_data_valid ? 1 : 0, expr_data_valid ? 1 : 0);

    // cluster_data_valid = (module && module->cluster_data && 
    //                           module->cluster_data->clusters) ? true : false;
    // mapping_data_valid = (module && module->mapping_data && 
    //                           module->mapping_data->clusters) ? true : false;
    // expr_data_valid = (module && module->expr_data && 
    //                        module->expr_data->expressions) ? true : false;

    // printf("before prepare bytecode2: cluster_data_valid: %d, mapping_data_valid: %d, expr_data_valid: %d\n",
    //        cluster_data_valid ? 1 : 0, mapping_data_valid ? 1 : 0, expr_data_valid ? 1 : 0);
    ...
    for (i = 0; i < module->function_count; i++) {
...
#ifdef DEBUG
        printf("function %d : number of local var: %d\n", i, func->local_count);
#endif
        ...
        // printf("func->code: %p, func->code_size: %u, buf_code_end: %p, function idx: %d\n",
        //     func->code, func->code_size, buf_code_end, i);
        ...
        if (i == module->function_count - 1
            ...
            // print func->code + func->code_size and buf_code_end
            // printf("func->code + func->code_size: %p, buf_code_end: %p\n",
            //     func->code + func->code_size, buf_code_end);

            set_error_buf(error_buf, error_buf_size,
                          "code section size mismatch");
            // print func->code which is uint8 *code;
            // printf("code: ");
            // for (int j = 0; j < func->code_size; j++) {
            //     printf("%02x ", func->code[j]);
            // }
        }
    }
...
// static uint32
// get_opcode_operand_size_simple(uint8 opcode, const uint8 *p, const uint8 *p_end)
// {
//     const uint8 *p_orig = p;
    
//     switch (opcode) {
//         case 0x01: case 0x05: case 0x0B: case 0x0F:
//         case 0x1A: case 0x1B: case 0x45:
...
//         case 0x24: case 0x41: case 0x42:
//             while (p < p_end && (*p & 0x80))
//                 p++;
//             if (p < p_end)
//                 p++;
//             return (uint32)(p - p_orig);
...
get_opcode_operand_size_simple(uint8 opcode, const uint8 *p, const uint8 *p_end)
{
    ...
    switch (opcode) {
        ...
        case WASM_OP_UNREACHABLE:      /* 0x00 */
        case WASM_OP_NOP:              /* 0x01 */
        case WASM_OP_ELSE:             /* 0x05 */
        case WASM_OP_END:              /* 0x0B */
        case WASM_OP_RETURN:           /* 0x0F */
        case WASM_OP_DROP:             /* 0x1A */
        case WASM_OP_SELECT:           /* 0x1B */
        case WASM_OP_I32_EQZ:          /* 0x45 */
        case WASM_OP_I32_EQ:           /* 0x46 */
        case WASM_OP_I32_NE:           /* 0x47 */
        case WASM_OP_I32_LT_S:         /* 0x48 */
        case WASM_OP_I32_LT_U:         /* 0x49 */
        case WASM_OP_I32_GT_S:         /* 0x4A */
        case WASM_OP_I32_GT_U:         /* 0x4B */
        case WASM_OP_I32_LE_S:         /* 0x4C */
        case WASM_OP_I32_LE_U:         /* 0x4D */
        case WASM_OP_I32_GE_S:         /* 0x4E */
        case WASM_OP_I32_GE_U:         /* 0x4F */
        case WASM_OP_I64_EQZ:          /* 0x50 */
        case WASM_OP_I64_EQ:           /* 0x51 */
        case WASM_OP_I64_NE:           /* 0x52 */
        case WASM_OP_I64_LT_S:         /* 0x53 */
        case WASM_OP_I64_LT_U:         /* 0x54 */
        case WASM_OP_I64_GT_S:         /* 0x55 */
        case WASM_OP_I64_GT_U:         /* 0x56 */
        case WASM_OP_I64_LE_S:         /* 0x57 */
        case WASM_OP_I64_LE_U:         /* 0x58 */
        case WASM_OP_I64_GE_S:         /* 0x59 */
        case WASM_OP_I64_GE_U:         /* 0x5A */
        case WASM_OP_F32_EQ:           /* 0x5B */
        case WASM_OP_F32_NE:           /* 0x5C */
        case WASM_OP_F32_LT:           /* 0x5D */
        case WASM_OP_F32_GT:           /* 0x5E */
        case WASM_OP_F32_LE:           /* 0x5F */
        case WASM_OP_F32_GE:           /* 0x60 */
        case WASM_OP_F64_EQ:           /* 0x61 */
        case WASM_OP_F64_NE:           /* 0x62 */
        case WASM_OP_F64_LT:           /* 0x63 */
        case WASM_OP_F64_GT:           /* 0x64 */
        case WASM_OP_F64_LE:           /* 0x65 */
        case WASM_OP_F64_GE:           /* 0x66 */
        case WASM_OP_I32_CLZ:          /* 0x67 */
        case WASM_OP_I32_CTZ:          /* 0x68 */
        case WASM_OP_I32_POPCNT:       /* 0x69 */
        case WASM_OP_I32_ADD:          /* 0x6A */
        case WASM_OP_I32_SUB:          /* 0x6B */
        case WASM_OP_I32_MUL:          /* 0x6C */
        case WASM_OP_I32_DIV_S:        /* 0x6D */
        case WASM_OP_I32_DIV_U:        /* 0x6E */
        case WASM_OP_I32_REM_S:        /* 0x6F */
        case WASM_OP_I32_REM_U:        /* 0x70 */
        case WASM_OP_I32_AND:          /* 0x71 */
        case WASM_OP_I32_OR:           /* 0x72 */
        case WASM_OP_I32_XOR:          /* 0x73 */
        case WASM_OP_I32_SHL:          /* 0x74 */
        case WASM_OP_I32_SHR_S:        /* 0x75 */
        case WASM_OP_I32_SHR_U:        /* 0x76 */
        case WASM_OP_I32_ROTL:         /* 0x77 */
        case WASM_OP_I32_ROTR:         /* 0x78 */
        case WASM_OP_I64_CLZ:          /* 0x79 */
        case WASM_OP_I64_CTZ:          /* 0x7A */
        case WASM_OP_I64_POPCNT:       /* 0x7B */
        case WASM_OP_I64_ADD:          /* 0x7C */
        case WASM_OP_I64_SUB:          /* 0x7D */
        case WASM_OP_I64_MUL:          /* 0x7E */
        case WASM_OP_I64_DIV_S:        /* 0x7F */
        case WASM_OP_I64_DIV_U:        /* 0x80 */
        case WASM_OP_I64_REM_S:        /* 0x81 */
        case WASM_OP_I64_REM_U:        /* 0x82 */
        case WASM_OP_I64_AND:          /* 0x83 */
        case WASM_OP_I64_OR:           /* 0x84 */
        case WASM_OP_I64_XOR:          /* 0x85 */
        case WASM_OP_I64_SHL:          /* 0x86 */
        case WASM_OP_I64_SHR_S:        /* 0x87 */
        case WASM_OP_I64_SHR_U:        /* 0x88 */
        case WASM_OP_I64_ROTL:         /* 0x89 */
        case WASM_OP_I64_ROTR:         /* 0x8A */
        case WASM_OP_F32_ABS:          /* 0x8B */
        case WASM_OP_F32_NEG:          /* 0x8C */
        case WASM_OP_F32_CEIL:         /* 0x8D */
        case WASM_OP_F32_FLOOR:        /* 0x8E */
        case WASM_OP_F32_TRUNC:        /* 0x8F */
        case WASM_OP_F32_NEAREST:      /* 0x90 */
        case WASM_OP_F32_SQRT:         /* 0x91 */
        case WASM_OP_F32_ADD:          /* 0x92 */
        case WASM_OP_F32_SUB:          /* 0x93 */
        case WASM_OP_F32_MUL:          /* 0x94 */
        case WASM_OP_F32_DIV:          /* 0x95 */
        case WASM_OP_F32_MIN:          /* 0x96 */
        case WASM_OP_F32_MAX:          /* 0x97 */
        case WASM_OP_F32_COPYSIGN:     /* 0x98 */
        case WASM_OP_F64_ABS:          /* 0x99 */
        case WASM_OP_F64_NEG:          /* 0x9A */
        case WASM_OP_F64_CEIL:         /* 0x9B */
        case WASM_OP_F64_FLOOR:        /* 0x9C */
        case WASM_OP_F64_TRUNC:        /* 0x9D */
        case WASM_OP_F64_NEAREST:      /* 0x9E */
        case WASM_OP_F64_SQRT:         /* 0x9F */
        case WASM_OP_F64_ADD:          /* 0xA0 */
        case WASM_OP_F64_SUB:          /* 0xA1 */
        case WASM_OP_F64_MUL:          /* 0xA2 */
        case WASM_OP_F64_DIV:          /* 0xA3 */
        case WASM_OP_F64_MIN:          /* 0xA4 */
        case WASM_OP_F64_MAX:          /* 0xA5 */
        case WASM_OP_F64_COPYSIGN:     /* 0xA6 */
        case WASM_OP_I32_WRAP_I64:     /* 0xA7 */
        case WASM_OP_I32_TRUNC_S_F32:  /* 0xA8 */
        case WASM_OP_I32_TRUNC_U_F32:  /* 0xA9 */
        case WASM_OP_I32_TRUNC_S_F64:  /* 0xAA */
        case WASM_OP_I32_TRUNC_U_F64:  /* 0xAB */
        case WASM_OP_I64_EXTEND_S_I32: /* 0xAC */
        case WASM_OP_I64_EXTEND_U_I32: /* 0xAD */
        case WASM_OP_I64_TRUNC_S_F32:  /* 0xAE */
        case WASM_OP_I64_TRUNC_U_F32:  /* 0xAF */
        case WASM_OP_I64_TRUNC_S_F64:  /* 0xB0 */
        case WASM_OP_I64_TRUNC_U_F64:  /* 0xB1 */
        case WASM_OP_F32_CONVERT_S_I32:/* 0xB2 */
        case WASM_OP_F32_CONVERT_U_I32:/* 0xB3 */
        case WASM_OP_F32_CONVERT_S_I64:/* 0xB4 */
        case WASM_OP_F32_CONVERT_U_I64:/* 0xB5 */
        case WASM_OP_F32_DEMOTE_F64:   /* 0xB6 */
        case WASM_OP_F64_CONVERT_S_I32:/* 0xB7 */
        case WASM_OP_F64_CONVERT_U_I32:/* 0xB8 */
        case WASM_OP_F64_CONVERT_S_I64:/* 0xB9 */
        case WASM_OP_F64_CONVERT_U_I64:/* 0xBA */
        case WASM_OP_F64_PROMOTE_F32:  /* 0xBB */
        case WASM_OP_I32_REINTERPRET_F32: /* 0xBC */
        case WASM_OP_I64_REINTERPRET_F64: /* 0xBD */
        case WASM_OP_F32_REINTERPRET_I32: /* 0xBE */
        case WASM_OP_F64_REINTERPRET_I64: /* 0xBF */
            return 0;
            
        case WASM_OP_CALL:             /* 0x10 */
            while (p < p_end && (*p & 0x80))
                p++;
            if (p < p_end)
                p++;
        ...
        case WASM_OP_BR:               /* 0x0C */
        case WASM_OP_BR_IF:            /* 0x0D */
            while (p < p_end && (*p & 0x80))
                p++;
            if (p < p_end)
                p++;
        ...
        case WASM_OP_BLOCK:            /* 0x02 */
        case WASM_OP_LOOP:             /* 0x03 */
        case WASM_OP_IF:               /* 0x04 */
            if (p < p_end) {
                // must be a block  after if 
                // block with return type or none
                if (*p == 0x40 || (*p >= 0x7C && *p <= 0x7F)) {
                    p++;
                }
                else {
                    while (p < p_end && (*p & 0x80))
        ...
        case WASM_OP_GET_LOCAL:        /* 0x20 */
        case WASM_OP_SET_LOCAL:        /* 0x21 */
        case WASM_OP_TEE_LOCAL:        /* 0x22 */
        case WASM_OP_GET_GLOBAL:       /* 0x23 */
        case WASM_OP_SET_GLOBAL:       /* 0x24 */
            while (p < p_end && (*p & 0x80))
                p++;
            if (p < p_end)
                p++;
        ...
        case WASM_OP_I32_LOAD:         /* 0x28 */
        case WASM_OP_I64_LOAD:         /* 0x29 */
        case WASM_OP_F32_LOAD:         /* 0x2A */
        case WASM_OP_F64_LOAD:         /* 0x2B */
        case WASM_OP_I32_LOAD8_S:      /* 0x2C */
        case WASM_OP_I32_LOAD8_U:      /* 0x2D */
        case WASM_OP_I32_LOAD16_S:     /* 0x2E */
        case WASM_OP_I32_LOAD16_U:     /* 0x2F */
        case WASM_OP_I64_LOAD8_S:      /* 0x30 */
        case WASM_OP_I64_LOAD8_U:      /* 0x31 */
        case WASM_OP_I64_LOAD16_S:     /* 0x32 */
        case WASM_OP_I64_LOAD16_U:     /* 0x33 */
        case WASM_OP_I64_LOAD32_S:     /* 0x34 */
        case WASM_OP_I64_LOAD32_U:     /* 0x35 */
        case WASM_OP_I32_STORE:        /* 0x36 */
        case WASM_OP_I64_STORE:        /* 0x37 */
        case WASM_OP_F32_STORE:        /* 0x38 */
        case WASM_OP_F64_STORE:        /* 0x39 */
        case WASM_OP_I32_STORE8:       /* 0x3A */
        case WASM_OP_I32_STORE16:      /* 0x3B */
        case WASM_OP_I64_STORE8:       /* 0x3C */
        case WASM_OP_I64_STORE16:      /* 0x3D */
        case WASM_OP_I64_STORE32:      /* 0x3E */
            for (i = 0; i < 2; i++) {
                while (p < p_end && (*p & 0x80))
                    p++;
                if (p < p_end)
        ...
        case WASM_OP_MEMORY_SIZE:      /* 0x3F */
        case WASM_OP_MEMORY_GROW:      /* 0x40 */
            if (p < p_end)
                p++;
            return (uint32)(p - p_orig);
            
        case WASM_OP_I32_CONST:        /* 0x41 */
            while (p < p_end && (*p & 0x80))
                p++;
            if (p < p_end)
                p++;
        ...
        case WASM_OP_F32_CONST:        /* 0x43 */
            p += 4;
            return 4;
            
        case WASM_OP_F64_CONST:        /* 0x44 */
            p += 8;
            return 8;
        ...
        default:
            /* For other opcodes, assume no operands */
            return 0;
    }
}
...
re_scan:
   ...
   // add: handle opcode replacement
    uint8 *code_end_addr = func->code + func->code_size;
    bool is_custom_opcode = false;
    uint32 custom_opcode_index = 0;
    uint32 cluster_idx = 0;
    bool found_in_cluster = false;
    uint32 expr_idx = 0;
    Expression *expr = NULL;

    // add: variables for code refactoring
    uint8 *new_code = NULL;
    uint32 new_code_size = 0;
    uint32 new_code_capacity = 0;
    uint32 offset = 0;

    // check if the current function is in a cluster
    if (module->cluster_data) {
        for (i = 0; i < module->cluster_data->num_clusters; i++) {
            Cluster *cluster = &module->cluster_data->clusters[i];
            for (uint32 j = 0; j < cluster->num_functions; j++) {
                if (cluster->functions[j] == cur_func_idx) {
                    cluster_idx = i;
...
#ifdef DEBUG
    printf("found_in_cluster: %d\n", found_in_cluster); 
#endif

    // if the current function is in a cluster, and the mapping_data and expr_data are not NULL
    if (found_in_cluster && module->mapping_data && module->expr_data) {
        // first pass: calculate the size of the code after replacement
        new_code_size = 0;
        p = func->code;
        while (p < p_end) {
            opcode = *p;
            ...
            is_custom_opcode = false;
            
            // custom op validation 
            if ((opcode >= newOP0 && opcode <= newOP4) ||
                (opcode >= newOP5 && opcode <= newOP12) ||
                (opcode >= newOP13 && opcode <= newOP15) ||
                (opcode == newOP16) ||
                (opcode >= newOP17 && opcode <= newOP27) ||
                (opcode >= newOP28 && opcode <= newOP68)) {
                
                // calculate the opcode index
                if (opcode >= newOP0 && opcode <= newOP4) {
                    custom_opcode_index = opcode - newOP0;
                }
                else if (opcode >= newOP5 && opcode <= newOP12) {
                    custom_opcode_index = (opcode - newOP5) + 5;
                }
                else if (opcode >= newOP13 && opcode <= newOP15) {
                    custom_opcode_index = (opcode - newOP13) + 13;
                }
                else if (opcode == newOP16) {
                    custom_opcode_index = 16;
                }
                else if (opcode >= newOP17 && opcode <= newOP27) {
                    custom_opcode_index = (opcode - newOP17) + 17;
                }   
                else if (opcode >= newOP28 && opcode <= newOP68) {
                    custom_opcode_index = (opcode - newOP28) + 28;
                }
                
                is_custom_opcode = true;
#ifdef DEBUG
                printf("--------------------------------\n");
                printf("is_custom_opcode: %d\n", is_custom_opcode); 
                // print the opcode 
                printf("opcode: 0x%x\n", opcode);

                printf("custom_opcode_index: %d\n", custom_opcode_index);
                printf("--------------------------------\n");
#endif
            }
            ...
            if (is_custom_opcode) {
                // check cluster index
                if (cluster_idx < module->mapping_data->num_clusters) {
                    ClusterMapping *mapping = &module->mapping_data->clusters[cluster_idx];
                    
                    // check opcode index
                    if (custom_opcode_index < mapping->num_expressions) {
                        expr_idx = mapping->expression_indices[custom_opcode_index];
                        
                        // check expression index
                        if (expr_idx < module->expr_data->num_expressions) {
                            expr = &module->expr_data->expressions[expr_idx];
                            
                            // calculate the size change after replacement
                            new_code_size += expr->size;
                            continue;
                        }
                    }
                }
                
                // if the expression is not found, keep the original
                new_code_size += 1;
            }
            else {
                // not a custom opcode, keep the original and its operands
                uint32 operand_size = get_opcode_operand_size_simple(opcode, p, p_end);

#ifdef DEBUG
                // printf("opcode1: 0x%x\n", opcode);
                // printf("operand_size1: %d\n", operand_size);
#endif 
                new_code_size += 1 + operand_size; 
        ...
        new_code_capacity = new_code_size;
        new_code = wasm_runtime_malloc(new_code_capacity);
        if (!new_code) {
            set_error_buf(error_buf, error_buf_size, "out of memory");
            return false;
        }
        ...
        while (p < code_end_addr) {
            ...
            // check if it is a custom opcode
            is_custom_opcode = false;
            
            // check if it is in the range of custom opcode
            if ((opcode >= newOP0 && opcode <= newOP4) ||
                (opcode >= newOP5 && opcode <= newOP12) ||
                (opcode >= newOP13 && opcode <= newOP15) ||
                (opcode == newOP16) ||
            ...
            if (is_custom_opcode) {
                if (cluster_idx < module->mapping_data->num_clusters) {
                    ...
                    if (custom_opcode_index < mapping->num_expressions) {
                        ...
                        if (expr_idx < module->expr_data->num_expressions) {
                            ...
                            // print custom_opcode_index
#ifdef DEBUG
                            printf("--------------------------------\n");
                            printf("opcode: 0x%x\n : custom_opcode_index: %d\n", opcode, custom_opcode_index);
                            printf("expr encoding: \n");
                            for (i = 0; i < expr->size; i++) {
                                printf("0x%x ", expr->encoding[i]);
                            }   
                            printf("\n");
                            printf("--------------------------------\n");
#endif 
                            bh_memcpy_s(new_code + offset, new_code_capacity - offset, 
                                       expr->encoding, expr->size);
                            offset += expr->size;
                            continue;
                        }
                    }
                ...
                new_code[offset++] = opcode;
            }
            else {
                // Not a custom opcode, copy the original opcode and its operands
                new_code[offset++] = opcode;
        ...
        func->code = new_code;
        func->code_size = new_code_size;
        
        // reset pointer for subsequent processing
        p = func->code;
        p_end = func->code + func->code_size;
        code_end_addr = p_end;
    }
    ...
    while (p < p_end) {
        ...
        emit_label(opcode);
#endif
// modification entry point opcode repalcement
        switch (opcode) {
            case WASM_OP_UNREACHABLE:
                RESET_STACK();
    ...
    //     for (i = 0; i < func->code_size; i++) {
    //         // if (i % 16 == 0) {
    //         //     printf("\n");
    //         // }
    //         printf("%x ", func->code[i]);
    //     }
    //     printf("\n");
    // }
